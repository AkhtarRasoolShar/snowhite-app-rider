...
# Revert states
states_to_revert = """    var selectedOrderForReview by remember { mutableStateOf<RiderOrder?>(null) }
...
content = content.replace(states_to_revert, "    var selectedOrderForReview by remember { mutableStateOf<RiderOrder?>(null) }")

# Revert items(sortedOrders) to items(orders) in MyOrdersScreen and RadarScreen
content = content.replace("items(sortedOrders) { order ->", "items(orders) { order ->")

# Revert old_else_block in MyOrdersScreen
bad_else_block = """            } else {
                Row(
...
good_else_block = """            } else {
                LazyColumn("""

content = content.replace(bad_else_block, good_else_block)
